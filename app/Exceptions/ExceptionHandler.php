...
use Tymon\JWTAuth\Exceptions\{JWTException, UserNotDefinedException, TokenExpiredException};
...
class ExceptionHandler
{
    ...
    ): JsonResponse
    {
        ...
        return self::responseError([
            'error' => [
                ...
                'message'   => $e->getMessage() ?? 'Authentication failed. Please log in again.',
    ...
    {
        $this->logException($e, 'JWT error');

        return self::responseError([
            'error' => [
                'type'      => $this->getExceptionClass($e),
                'status'    => HttpCodes::UNAUTHORIZED,
                'message'   => $e->getMessage() ?? 'Failed to create token. Please try again.',
                'timestamp' => now()->toISOString(),
            ]
        ], HttpCodes::UNAUTHORIZED);
    }
     ...
     */
    public function handleJWTExpiredException(TokenExpiredException $e, Request $request): JsonResponse
    {
        $this->logException($e, 'JWT expiration error');

        return self::responseError([
            'error' => [
                'type'      => $this->getExceptionClass($e),
                'status'    => HttpCodes::UNAUTHORIZED,
                'message'   => $e->getMessage() ?? 'Token has expired and can no longer be refreshed',
                'timestamp' => now()->toISOString(),
            ]
        ], HttpCodes::UNAUTHORIZED);
    }
